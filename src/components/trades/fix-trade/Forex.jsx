...
const Forex = ({ account, prices, action, user, fiat }) => {
  // toast
  ...
  function handleAmount(e) {
    ...
    if (Number(value) && Number(value) > 0) {
      ...
      if (Number(value) > usdBalance) {
        ...
        setAmountError(true);
        setToolTipMessage(
          `Forex trading uses the USD balance. You have selected an amount that is above the user's current USD balance.`
        );
      } else {
        setAmountError(false);
      }
